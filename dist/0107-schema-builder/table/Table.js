...
class Table {
    ...
    constructor(options) {
        ...
        if (options) {
            ...
            if (options.foreignKeys)
                this.foreignKeys = options.foreignKeys.map(foreignKey => new TableForeignKey_1.TableForeignKey(foreignKey));
            if (options.uniques)
    ...
    static create(entityMetadata, driver) {
        const options = {
            ...
            columns: entityMetadata.columns.filter(column => column).map(column => TableUtils_1.TableUtils.createTableColumnOptions(column, driver)),
