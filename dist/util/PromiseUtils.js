...
class PromiseUtils {
    ...
    static async runInSequenceWithPageSize(collection, callback, pageSize = 1) {
        for (let i = 0; i * pageSize < collection.length; i++) {
            await Promise.all(collection.slice(i * pageSize, (i + 1) * pageSize).map(callback));
        }
    }
}
